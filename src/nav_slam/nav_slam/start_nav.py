...
class PurePursuitController:
    ...
    def calculate_steering_angle(self, vehicle_pose, path_points):
        ...
        for i in range(len(path_points)):
            ...
            if lookahead_point_idx >= len(path_points):
...
class PathFollowingNode(Node):
    def __init__(self):
        ...
        # 创建路径点订阅者
        self.path_subscriber = self.create_subscription(Path, '/path2', self.path_callback, 10)
        # 变量初始化
    ...
    def odometry_callback(self, msg):
        ...
        if distance_to_end < 0.2:  # 0.2m作为接近阈值
            ...
            # 根据转向角大小调整线速度
            speed = max(0.1, 1.5 - 1.5 * math.sin(0.6 * abs(steering_angle)))

            # 将纯追踪的航向误差转换为角速度（差速驱动模型）
            # angular_z = 2 * v * sin(alpha) / Ld
            angular_z = 2.0 * speed * math.sin(steering_angle) / self.pure_pursuit.lookahead_distance
